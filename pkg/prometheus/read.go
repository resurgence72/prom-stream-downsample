...
func (p *Prometheus) RemoteRead(
	...
	// 发送请求
	httpReq, err := http.NewRequestWithContext(context.Background(), "POST", p.remoteReadURL, bytes.NewReader(snappy.Encode(nil, data)))
	if err != nil {
		logrus.Error(err)
		return nil, 0, err
	}
	...
	if strings.HasPrefix(contentType, "application/x-protobuf") {
		...
		return p.sampleRemoteRead(resp)
		...
		return p.streamRemoteRead(resp)
	}
	return nil, 0, fmt.Errorf("unknown response type: %s", contentType)
}

func (p *Prometheus) sampleRemoteRead(resp *http.Response) ([]pb.TimeSeries, int64, error) {
...
func (p *Prometheus) streamRemoteRead(resp *http.Response) ([]pb.TimeSeries, int64, error) {
	// 解码响应
